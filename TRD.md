...
02-03의 설계 검토는 파일 책임·요구사항 대응·시간/입력/충돌 경계를 대상으로 했으며 당시 실제 구현·명령 실행·브라우저·배포 검증은 미수행이었다. 이후 구현과 자동 브라우저 검증은 실제 실행 기록으로 구분한다. 사람의 직접 플레이·OS 수준 포커스·공개 배포 검증은 현재도 unverified다.
