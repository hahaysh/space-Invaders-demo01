...
## 6. CHG-02 검사 계획 (08-01)

| TC | 기준 | 절차·기대 결과 |
|---|---|---|
| TC-D01 | AC1/9 | 미지정/undefined 생성은 normal64. 세 유효 값과 지원하지 않는 문자열/null/타입/상속 속성 이름을 구분하고 잘못된 값은 오류 |
| TC-D02 | AC2/3/4 | title/won/lost의 다음 선택만 변경, start/restart에서 적용. playing/paused는 유효한 변경 시도 무시, 이번 게임 설정 보존 |
| TC-D03 | AC6/10 | 0.1초 이동 3.2/6.4/9.6px. 각 속도의 양쪽 벽 직전/도달/초과에서 한 번 반전·24하강. normal 기존 수치 회귀 |
| TC-D04 | AC3/4/5/8/9 | 실제 label/select 키보드·Enter 비시작·P 비처리·표시, 진행/정지 잠금과 DOM 조작 우회 차단, 종료 후 선택·재시작 적용, 새로고침 normal, 잘못된 값의 한국어 오류 |
| TC-D05 | AC7 | 세 난이도에서 P·60초 동결·재개 시간/입력 해제. 상태 전환 뒤 고정 난이도 보존 |
